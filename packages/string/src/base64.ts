...
class Base64 {
  ...
  decode(encoded: string | Buffer, encoding: BufferEncoding = 'utf-8', strict = false): string | null {
  ...
  urlDecode(encoded: string | Buffer, encoding: BufferEncoding = 'utf-8', strict = false): string | null {
    if (Buffer.isBuffer(encoded)) {
      return encoded.toString(encoding);
    }
