 ...
 * underlying PG pool.
 */
export function eventSourcing(options: EventSourcingDotOptions): DotPip<EventSourcingServices> {
  ...
  return defineDotPip<EventSourcingServices>({
    name,
    version: '0.1.0',
    ...
    boot() {
      ...
      if (dbUrl === undefined) {
        throw new DotPipError({
          ...
          message: `[${name}] Event Store database URL is not configured.`,
